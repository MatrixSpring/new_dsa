...
## [Unreleased]
- [文档] 新增 `docs/crawler-llm-parse-integration.md`（DSA-CRAWL-LLM-MERGE-V1.0）：自动爬虫 + 长文本解析全自动流水线整合方案；含 GitHub 爬虫项目真实核查（cninfo-crawler / China_stock_announcement / a-stock-data / Crawl4AI / akshare / FinGPT / WaterCrawl）、四层架构、crawl_service 接入点（data_provider/extended、runtime_scheduler）、排期与合规边界。
...
- [新功能] 新增三聚合页（设计 §2.1/§2.2/§2.3）：全球动态 / 国内动态 / 单日动态。复用现有 dashboard 7 组端点 + intelligence/items，按 variant 并行拉取情报与仪表盘面板；新增 `types/dynamics.ts`、`api/dynamics.ts`、`components/dynamics/DynamicsViews.tsx`、`pages/DynamicsCenterPage.tsx` 及三个薄包装页，注册 `/global-dynamics` `/domestic-dynamics` `/daily-dynamics` 路由与侧边导航（含 zh/en i18n）；后端 `scripts/serve_dynamics.py` + 前端 `scripts/verify_dynamics_display.tsx` 验证「请求→数据→显示」全链路。
- [新功能] 长文本深度解析模块（DSA-OPT-LLM-001）：新增 `core/llm_parse_service.py`（5 子模块：分层拆解/多文档对比/隐藏约束挖掘/长期规划提取/隐性风险挖掘，复用 `llm/gateway.py` 统一 LLM 通道，无 API key 时确定性启发式降级）与 `api/v1/endpoints/llm_parse.py`（`/api/v1/llm-parse/{document,compare,constraints,long-term}`）；前端新增 `types/llmParse.ts`、`api/llmParse.ts`、`components/llm-parse/{LlmParseModal,ParseResultView}.tsx` 并在动态页接入「LLM 深度解读」入口；`scripts/serve_llm_parse.py` + `scripts/verify_llm_parse_display.tsx` 验证全链路（请求→数据→显示）。
- [新功能] 运维后台定时任务可视化（六段可视化之一）：新增 `src/services/job_registry.py`（6 个自动任务定义，daily/weekly/interval 触发，next_run 计算，JSON 持久化）与 `api/v1/endpoints/scheduler.py`（`/api/v1/scheduler/jobs`、`/jobs/{id}/toggle`、`/jobs/{id}/run`）；前端新增 `types/scheduler.ts`、`api/scheduler.ts`、`components/scheduler/JobBoard.tsx`、`pages/SchedulerAdminPage.tsx` 并注册 `/scheduler` 路由与侧边导航（含 zh/en i18n）；`scripts/serve_scheduler.py` + `scripts/verify_scheduler_display.tsx` 验证全链路。
- [新功能] 预测复盘归因自动打分（三层复盘）：新增 `core/review_scorer.py`（逐周期方向/区间命中 + 数据层/模型层/逻辑层归因 + 聚合报告）与 `api/v1/endpoints/review.py`（`/api/v1/review/score`、`/report`）；前端新增 `types/review.ts`、`api/review.ts`、`components/review/ReviewBoard.tsx`、`pages/ReviewCenterPage.tsx` 并注册 `/review-center` 路由与侧边导航（含 zh/en i18n）；`scripts/serve_review.py` + `scripts/verify_review_display.tsx` 验证全链路。
...
- [新功能] DSA 传导引擎补设计 §3.1 规则并接入三情景并行推演（#12）：`src/industry_chain_propagation.py` 的 `propagate_shock` 增加 `max_depth=20` 深度上限、`bidirectional_decay=0.85` 双向逐跳衰减、`bearish_decay=0.7` 利空衰减、`coeff` 区间钳制 `[0,1]`、`use_overrides` 接入 `chain_edge_override` 覆盖系数、`factor_weights` 回显；端点 `propagate` 读取 `dsa_global_params`（recursion_depth/coeff_threshold/bearish_weight）作为默认并支持请求体覆盖，新增 `POST /industry-chains/{chain_id}/propagate-scenarios` 输出基准/乐观/悲观三情景；前端新增 `types/industryChain.ts`/`api/industryChain.ts` 类型与接口、`pages/DsaEnginePage.tsx`（引擎参数联动全局参数 + 单传导 + 三情景对比）并注册 `/dsa-engine` 路由与侧边导航（含 zh/en i18n）；`scripts/serve_dsa_engine.py` + `scripts/verify_dsa_engine_display.tsx` 验证「请求→数据→显示」全链路。
- [新功能] 大涨个股反向新闻归因回溯子系统（DSA-BACKTRACE-V1.0，#13）：新增外挂微服务「反向归因」，覆盖 SRS 五层架构——① 大涨标的筛选（`backtrace_screen_pool` 表 + `POST /backtrace/screen`）、② 拉升前历史资讯回溯与时间过滤（`backtrace_news_docs` 表 + `POST /backtrace/backtrack`，事后新闻强制剔除）、③ 启发式归因数学模型 + ④ 标准化 §3.4 JSON 输出（`backtrace_attributions` 表 + `POST /backtrace/attribute`，权重合计 100%、三类驱动、置信度、相似历史对标、DSA 调整建议）、⑤ DSA 系统联动（`backtrace_linkages` 表 + `POST /backtrace/link`，事件库入库/个股权重/产业链系数/四周期预测重算/案例沉淀）；内置 §3.3 固定 Prompt 常量供 LLM 远程归因外挂，决策权坚守（量化权重由数学模型输出、LLM 仅做信息梳理）；前端新增 `types/backtrace.ts`/`api/backtrace.ts`/`pages/BacktracePage.tsx`（大涨回溯池→反向归因→防幻觉三护栏→DSA 联动）并注册 `/backtrace` 路由与侧边导航（含 zh/en i18n）；`scripts/serve_backtrace.py` + `scripts/verify_backtrace_display.tsx` 验证「请求→数据→显示」全链路。
- [新功能] 反向归因子系统新增「批量板块复盘」（SRS §3.6，#14）：针对板块集体大涨批量回溯板块内个股共同前置事件，输出板块景气判断（景气主升/景气上行（分化）/情绪脉冲）/板块轮动逻辑/上下游传导链/共同前置事件分布/个股归因画像；新增 `backtrace_sector_reviews` 伴随表与 `POST /backtrace/sector-review`、`GET /backtrace/sector-reviews` 端点（`batch_sector_review` 聚合引擎，确定性 mock 板块成分股与传导链，真实环境替换为行情板块成分股接口）；前端 `BacktracePage.tsx` 新增「② 批量板块复盘」Tab（板块选择→运行→景气 badge/轮动逻辑/传导链/共同事件/个股表），`scripts/serve_backtrace.py` 与 `verify_backtrace_display.tsx` 已覆盖后端 request→data 与前端 SSR 全链路校验。
- [新功能] 反向归因子系统新增「归因有效性回测校验」（SRS §3.7，#15）：将某次结构化归因的驱动因子按关键词匹配历史同类行情样本库（确定性 mock，按业绩订单/产业政策/技术突破/机构研报/资金调研/情绪游资分桶），加权聚合历史胜率/平均 1 周·1 月涨幅/平均回撤/期望净收益，并据此反向修正该次归因置信度（规避事后强行归因）；新增 `backtrace_backtests` 伴随表、`POST /backtrace/backtest`、`GET /backtrace/backtests` 端点（`backtest_attribution` 引擎，真实环境替换为 Backtrader/Zipline/Janus-Q 行情回测框架按驱动标签检索）；前端 `BacktracePage.tsx` 新增「③ 归因回测校验」Tab（归因下拉选择→运行→有效性判定/核心指标卡/置信度修正/因子↔样本桶匹配明细），`scripts/serve_backtrace.py` 与 `verify_backtrace_display.tsx` 已覆盖后端 request→data 与前端 SSR 全链路校验（`DISPLAY_OK checks=39`）。
- [新功能] 反向归因子系统新增「Agent 自主深挖小众突发事件」（#16，增强模块）：在反向回溯基础上让 Agent 主动扫描股价异动前窗口内的隐藏早期信号——机构调研/产业链异动/舆情小道消息/游资动向，按「时间临近度(0.5)+来源可信度(0.3)+相关度(0.2)」综合打分（0~100）、标记小众早期信号（提前≥10 日）、输出信号时间线并反哺 §3.4 归因的早期佐证强度；决策权坚守（全部数学打分，不依赖 LLM 主观臆断），沙箱无外网/无 LLM key 走确定性 mock，真实环境替换为调研平台/舆情监控/龙虎榜接口；新增 `backtrace_agent_signals` 伴随表、`POST /backtrace/agent-dig`、`GET /backtrace/agent-signals` 端点（`src/services/agent_signal_service.py` 的 `agent_dig` 引擎），前端 `BacktracePage.tsx` 新增「④ Agent 深挖信号」Tab（标的选择→运行→信号总数/小众早期 badge/类型分布/评分降序信号卡/拉升前时间线）；`scripts/serve_backtrace.py` 与 `verify_backtrace_display.tsx` 已覆盖后端 request→data 与前端 SSR 全链路校验（`DISPLAY_OK checks=52`）。
- [新功能] 反向归因子系统新增「高频上涨因子自动沉淀」（#17，增强模块）：把已验证的 §3.4 归因与 §3.7 回测结果按驱动因子聚合统计，自动沉淀为标准化「上涨因子库」（出现频次/历史胜率/平均涨幅/期望净收益/置信度，「置信度随频次单调上升」规避小样本过拟合），并反哺正向预判——输入早期信号（Agent 深挖的四类隐藏信号或任意文本）匹配因子库后输出置信度加权的历史上涨概率与建议动作（强信号/中性偏多/审慎三档）；形成「反向归因 → 因子沉淀 → 正向预判」闭环，决策权坚守（全部数学聚合/加权，不依赖 LLM 主观臆断）；新增 `backtrace_factor_library` 伴随表、`POST /backtrace/factor-mine`、`GET /backtrace/factor-library`、`POST /backtrace/factor-predict` 端点（`src/services/factor_library_service.py` 的 `mine_factors`/`list_factor_library`/`predict_with_factors` 引擎，预设基线因子 + DB 归因聚合，真实环境由生产库归因累积替代），前端 `BacktracePage.tsx` 新增「⑤ 上涨因子库」Tab（沉淀按钮→因子榜单/正向预判面板）；`scripts/serve_backtrace.py` 与 `verify_backtrace_display.tsx` 已覆盖后端 request→data 与前端 SSR 全链路校验（`DISPLAY_OK checks=65`）。
- [新功能] 反向归因子系统新增「因子库 → DSA 内核正向传导桥接」（#18，闭环增强，内核零改动）：把 #17 沉淀的标准化上涨因子库（按期望净收益排序、置信度过滤取 top-N）转为 DSA 引擎 `propagate_shock` 的 `factor_weights` 字典，并以「冲击幅度因子增益」（Σ(weight×期望净收益得分) 钳制 [0,0.5]）真实注入四周期正向传导，输出「基线 vs 因子增强」的最大冲击/影响环节/涉及公司提升与四周期（1w/2w/1m/6m）传导预测；内核 `propagate_shock` 的 `factor_weights` 当前仅回显，本桥接通过幅度增益真实影响传导、内核零改动，未来内核若支持 factor_weights 加权传导可直接消费同一字典（向后兼容）；新增 `src/services/factor_propagation_service.py`（`build_factor_weights`/`forecast_with_factors` 引擎）+ `POST /industry-chains/{chain_id}/factor-forecast` 端点（挂载于 industry_chain 路由，复用 `_get_chain_graph` 与 `propagate_shock`），前端 `BacktracePage.tsx` 新增「⑥ 因子传导 (内核)」Tab（产业链/冲击环节/幅度选择→运行→基线 vs 增强指标卡/四周期柱状预测图/注入因子权重明细）；`scripts/serve_backtrace.py`（已挂载 industry_chain 路由）与 `verify_backtrace_display.tsx` 已覆盖后端 request→data 与前端 SSR 全链路校验（`DISPLAY_OK checks=80`）。
- [新功能] 反向归因子系统新增「一键闭环」（#19，收尾闭环）：把已完成的 #16 Agent 深挖 / #17 因子正向预判 / #18 因子内核传导编排为单次调用的一条龙链路——`POST /backtrace/closed-loop` 接收 `stock_code`（chain_id 缺省按标的行业关键词推断产业链）→ 阶段一 `agent_dig` 扫描拉升前隐藏信号 → 阶段二 `predict_with_factors` 把信号类型对齐因子库输出置信度加权上涨概率 → 阶段三 `forecast_with_factors` 把命中因子转为桥接权重注入四周期正向传导，返回 `{dig, predict, propagate}` 三段式结论；内核零改动、决策权全部数学编排不依赖 LLM，沙箱确定性 mock；新增 `src/services/closed_loop_service.py`（`run_closed_loop` 引擎，复用产业链沙盘 JSON 与既有传导幅度增益通道），前端 `BacktracePage.tsx` 新增「⑦ 一键闭环」Tab（标的选择→运行→三段式阶段面板：隐藏信号/小众早期→预测概率与命中因子→基线 vs 增强指标卡与四周期柱状图与注入因子明细）；`scripts/serve_backtrace.py` 与 `verify_backtrace_display.tsx` 已覆盖后端 request→data 与前端 SSR 全链路校验（`DISPLAY_OK checks=95`）。至此反向归因子系统 7 大主干模块(#13~#15)+增强闭环(#16 Agent 深挖、#17 因子沉淀正向预判、#18 因子传导内核、#19 一键闭环)全闭环。
- [新功能] 反向归因子系统新增「自动化闭环预警扫描」（#20，生产化闭环）：把 #19 一键闭环编排为批量自动化预警服务——`POST /backtrace/closed-loop/scan` 默认扫描当日大涨回溯池（或显式 `watchlist`，显式空列表拒绝）、逐只跑闭环，综合「上涨概率(0.4)/内核增益(0.2)/小众早期信号(0.2)/最强单信号评分(0.2)」四项指标给出 0~1 综合预警评分并按评分降序，分级为「强信号·重点关注 / 中性·持续观察 / 弱信号·低关注」；`GET /backtrace/closed-loop/alerts` 返回最近批次预警结果，便于接入收盘后定时预警；决策权坚守（全部数学加权，不依赖 LLM）、内核零改动，沙箱确定性 mock；新增 `backtrace_scan_alerts` 伴随表与 `src/services/closed_loop_scan_service.py`（`scan_alerts`/`list_scan_alerts` 引擎），前端 `BacktracePage.tsx` 新增「⑧ 自动化闭环预警」Tab（运行→按综合评分降序的分级看板 + 评分分布条）；`scripts/serve_backtrace.py` 与 `verify_backtrace_display.tsx` 已覆盖后端 request→data 与前端 SSR 全链路校验（`DISPLAY_OK checks=104`）。
- [新功能] 反向归因子系统新增「闭环预警自动化调度」（#21，把 #20 接成定时/事件触发任务）：把 #20 批量扫描包装为可调度任务——后端 `src/services/closed_loop_scheduler_service.py`（`DEFAULT_CRON='30 15 * * 1-5'` 即收盘后 周一至周五 15:30、轻量 5 段 cron 匹配 `_match_field`/`cron_matches`/`should_trigger_now`、`get_schedule_config`/`set_schedule_config`（校验 5 段）、`run_scheduled_scan`（包装 scan_alerts 并落批次聚合：分级计数 + Top 标的与综合评分）、`get_scan_history`），`storage.py` 新增 `backtrace_scan_batches`（批次历史聚合）与 `backtrace_scan_schedule`（cron 单键配置）伴随表；端点 `POST /closed-loop/scan/run`（manual/schedule/event，落批次）、`GET /closed-loop/scan/history`、`GET|PUT /closed-loop/scan/schedule`（cron 校验）；前端 `BacktracePage.tsx` ⑧ Tab 新增「调度面板」（cron 可读提示 + 启停状态 + 立即运行入口）与「批次历史」表（触发方式/分级计数/Top 标的/运行时间）；新增无头入口 `scripts/run_backtrace_scan.py`（供 cron/CI/自动化调用，自补回溯池，输出单行 JSON 摘要），并注册 WorkBuddy 定时自动化（周一至周五 15:30 收盘后）触发该脚本并汇报强信号名单；决策权全数学、内核零改动、沙箱确定性 mock；`scripts/serve_backtrace.py` 与 `verify_backtrace_display.tsx` 已覆盖后端 request→data 与前端 SSR 全链路校验（`DISPLAY_OK checks=113`）。
- [新功能] 反向归因子系统新增「内核结构化边注入」（#22，把 #18 的全局幅度增益升级为按因子类别差异化增强对应产业链边的结构化注入）：`src/services/factor_propagation_service.py` 新增 `_FACTOR_CATEGORY_EDGE_MAP`（基本面事件驱动→supply/cost、资金筹码驱动→demand、题材情绪驱动→subst+demand）与 `_build_edge_overrides`（按命中因子类别×期望得分×通道权重聚合每类边增益，覆盖图谱对应边系数；内核把图当无向遍历，故每条边双向写入 `(source,target)`/`(target,source)` 两个方向确保正/反向遍历都能命中增强系数），`forecast_with_factors` 的增强视图改为 `use_overrides=True` + 覆盖系数注入（内核 `propagate_shock` 零改动，走既有的 `use_overrides` 通道），`boost` 语义升级为 `structuredBoost`（= 最强被注入通道的系数增益，0~1 钳制，不再恒为 0.5 上限而更具区分度）；返回新增 `structuredBoost`/`edgeOverrides`（被注入边明细：源/目标/边类型/基线系数/覆盖系数/增益/贡献类别）/`categoryEdgeContrib`（类别→边 贡献拆解）；前端 `types/backtrace.ts` 新增 `EdgeOverrideItem`/`CategoryEdgeContrib`、`BacktracePage.tsx` ⑥ 因子传导 Tab 新增「结构化边注入」区块（类别→边贡献卡 + 被注入边明细表）与概览 badge、闭环 Tab 增加结构化注入边 badge；决策权全数学、内核零改动、沙箱确定性 mock；`scripts/serve_backtrace.py` 放宽 #14 线性等式断言为「仅增不减」并补 structuredBoost/edgeOverrides/categoryEdgeContrib 断言，`verify_backtrace_display.tsx` 已覆盖 ⑥ 结构化注入区块与闭环 badge（`DISPLAY_OK checks=120`）。至此 DSA-BACKTRACE-V1.0（#13~#15 主干 + #16~#22 增强）全闭环；剩余可选方向：③ 真实环境适配（AkShare/cninfo/调研平台/龙虎榜替换 mock 信号源与因子累积）。
- [新功能] 反向归因子系统新增「可插拔实时数据源适配器」（#23，真实环境适配）：把 #20~#22 闭环预警链路中的「信号源/因子累积/大涨回溯池」从确定性 mock 升级为可切换生产数据源——新建 `src/services/market_data_provider.py`，定义统一接口 `BaseMarketProvider`（`get_surging_stocks`/`get_dragon_tiger_list`/`get_institution_surveys`），`MockMarketProvider`（复用 `backtrace_service._MOCK_POOL` 确定性返回，杜绝双 mock 漂移）与 `AkShareMarketProvider`（真实实现：涨幅榜 `stock_zh_a_spot_em` / 龙虎榜 `stock_lhb_detail_em` / 机构调研 `stock_jgdy_em`，均懒加载 akshare、缺失或失败抛 `ProviderUnavailable`）；`get_market_provider` 工厂按环境变量 `DSA_REALTIME_MARKET=1` 切换，真实不可用**优雅回退** mock 并记录原因；`describe_source` 返回 `{provider,label,mode,reason,surgingCount,envKey}`、`refresh_screen_pool` 用活跃数据源重写当日回溯池；`scan_alerts` 当 `watchlist=None` 且回溯池为空时自动 `refresh_screen_pool` 后重解析（真实环境自动拉取涨幅榜，沙箱池已填充故不改变既有断言）；端点 `GET /closed-loop/scan/source`、`POST /closed-loop/scan/refresh-pool`；前端 `types/backtrace.ts` 增 `DataSourceInfo`、`BacktracePage.tsx` ⑧ Tab 增「数据源」badge（实时 AkShare / 模拟 + provider 名）、live 模式 mount 加载 `dataSource`；决策权仍全数学（数据源只喂标的不介入打分）、内核零改动、切换零代码改动仅由环境变量控制；`scripts/serve_backtrace.py` 增 #23 断言（沙箱 mode=mock、surgingCount>0、refresh-pool count>0），`verify_backtrace_display.tsx` 覆盖数据源 badge（`DISPLAY_OK checks=122`）。至此 DSA-BACKTRACE-V1.0（#13~#15 主干 + #16~#23 增强）全闭环，原路线图三个可选方向（自动化调度/内核结构化注入/真实环境适配）均已落地。
- [新功能] 反向归因子系统新增「因子库数据驱动沉淀」（#24，把 #17 因子库从「仅预设基线」升级为「预设基线 + 生产真实归因累积」）：在 #19 一键闭环链路 `run_closed_loop` 中默认 `persist_attribution=True`，每次跑闭环都调用 `attribute()` 把真实反向归因落库 `backtrace_attributions`（失败兜底 warn 不影响主链路）；`mine_factors` 已有的 `_db_mined_factors` 据此从空累积变为真实来源，实现「反向归因 → 因子沉淀」真正闭环；新增 `factor_library_stats` 引擎（`presetCount`/`dbAttributionCount`/`minedFromDb`/`reinforced`/`libraryTotal`）与 `GET /factor-library/stats` 端点，量化对比基线因子与生产归因累积体量；前端 `types/backtrace.ts` 增 `FactorLibraryStats`、`api/backtrace.ts` 增 `factorLibraryStats`、`BacktracePage.tsx` ⑤ 因子库 Tab 新增「因子库累积统计」面板（基类预设因子/真实归因落库/DB 新发掘因子/被强化基线因子/因子库总数五指标 + 数据驱动说明）；决策权全数学、内核零改动、沙箱确定性 mock；`scripts/serve_backtrace.py` 增 #24 断言（闭环结果含 attribution、stats 端点 dbAttributionCount≥1、闭环后 factor-mine 的 minedFromDb≥1），`verify_backtrace_display.tsx` 覆盖累积统计面板（`DISPLAY_OK checks=129`）。
- [新功能] 反向归因子系统新增「可插拔公开披露数据源适配器」（#25，真实环境适配收尾：把与 #23 行情源正交的「基本面催化信号源」从确定性 mock 升级为 cninfo/财报/研报可切换源）：新建 `src/services/disclosure_provider.py`，定义统一接口 `BaseDisclosureProvider`（`get_disclosures`/`get_financial_reports`/`get_research_reports`），`MockDisclosureProvider`（确定性模板，仅引用大涨池内标的以保闭环可解析）与 `CninfoDisclosureProvider`（真实实现经 AkShare 懒加载：`stock_notice_report`/`stock_yjbb_em`/`stock_research_report_em`，缺失或失败抛 `ProviderUnavailable`）；`get_disclosure_provider` 工厂按 `DSA_REALTIME_DISCLOSURE=1` 切换、不可用优雅回退 mock；`describe_disclosure_source` 返回 `{provider,label,mode,reason,disclosureCount,financialCount,researchCount,envKey}`、`refresh_disclosure_pool` 重写 `backtrace_disclosures` 伴随表、`list_disclosure_pool` 查询；闭环预警扫描 `scan_alerts` 在 `watchlist=None` 时把披露池标的作为基本面筛选叠加（union，披露引入但不在大涨池的标的好名单登记进 `BacktraceScreenPool` 使 agent_dig 可解析），扫描结果新增 `disclosureCandidates` 与 per-alert `hasDisclosure`；端点 `GET /closed-loop/disclosure/source`、`POST /closed-loop/disclosure/refresh`、`GET /closed-loop/disclosures`；前端 `types/backtrace.ts` 增 `DisclosureItem`/`DisclosureSourceInfo`、`api/backtrace.ts` 增 `disclosureSource`/`disclosureRefresh`/`disclosures`、`BacktracePage.tsx` ⑧ Tab 增「公开披露」badge + 披露催化事件池面板（公告/业绩预告/重大合同/股权激励/财报/研报点评表 + 情绪着色）并把命中披露的预警在分级看板标「披露催化」；决策权全数学（披露源只喂基本面催化标的不介入打分）、内核零改动、切换零代码改动仅由环境变量控制、与 #23 行情源正交；`scripts/serve_backtrace.py` 增 #25 断言（source mode=mock 且 disclosureCount>0、refresh count>0、scan 叠加 disclosureCandidates≥1），`verify_backtrace_display.tsx` 覆盖披露面板与 badge（`DISPLAY_OK checks=138`）。
- [新功能] 反向归因子系统新增「公开披露源无头刷新入口」（#26，把 #25 真实环境适配接成 cron/CI 可调用任务）：新增 `scripts/run_disclosure_refresh.py`，不启动 Web 服务即可刷新 `backtrace_disclosures` 事件池（mock / cninfo 经 AkShare），支持 `--days`/`--stock-codes`/`--scan`/`--dry-run`/`--json`，复用 `#25` 既有 `disclosure_provider` 工厂与优雅回退逻辑，零代码改动切换数据源；`--scan` 时联动 `scan_alerts` 验证披露叠加（union，`disclosureCandidates`≥1、per-alert `hasDisclosure`）；`--dry-run` 仅探测数据源不写库；cron 友好退出码（0 成功 / 1 运行错误 / 2 已请求实时披露源却回退 mock 需运维关注）。已验证：默认 mock 刷新 12 条事件（6 公告+3 财报+3 研报）退出 0、`--scan` 闭环扫描总计 14 只披露叠加候选 6 只、`--dry-run` 退出 0、`DSA_REALTIME_DISCLOSURE=1` 缺失 akshare 回退 mock 退出 2。
- [新功能] 反向归因子系统新增「收盘后公开披露刷新 + 闭环预警」定时自动化（#27，把 #26 无头 CLI 接成定时任务）：注册 WorkBuddy 定时自动化（id `automation-1785668758105`，周一至周五 16:00 收盘后，独立于 #21 的 15:30 闭环扫描任务以避免重叠），触发 `scripts/run_disclosure_refresh.py --scan --json` 并汇报「披露催化 + 强信号」名单、数据源模式与披露事件数（公告/财报/研报）；实时源请求却回退 mock 时提示运维 `pip install akshare` 后重跑。至此 DSA-BACKTRACE-V1.0（#13~#15 主干 + #16~#27 增强）全闭环；原三大可选方向（自动化调度/内核结构化注入/真实环境适配）均已落地，并完成 #25/#26 的 cron/CI 无头调用与定时自动化收尾。
- [新功能] 反向归因子系统新增全新子系统「公开舆情（DSA-PUBLIC-OPINION-V1.0 #28，真实环境适配外延：把与 #23 行情源/#25 披露源均正交的「情绪面催化信号源」从确定性 mock 升级为头条爬虫 + FinBERT 可切换源）」：新建 `src/services/opinion_provider.py`，定义统一接口 `BaseOpinionProvider`（`get_opinions` 返回标准化 JSON：sentiment_score(-1~1)/heat_score(0~1)/stage(萌芽/发酵/狂热/退潮)/has_rumor/weight_suggest），`MockOpinionProvider`（确定性模板，仅引用大涨池内标的、覆盖利好/利空/谣言/四阶段多样性以驱动前端着色与风控展示）与 `ToutiaoOpinionProvider`（真实实现：crawl_service.toutiao_spider 头条爬虫 + FinBERT 本地推理 + 谣言降权 80%，缺失/失败抛 `ProviderUnavailable`）；`get_opinion_provider` 工厂按 `DSA_REALTIME_OPINION=1` 切换、不可用优雅回退 mock；`describe_opinion_source` 返回 `{provider,label,mode,reason,opinionCount,rumorCount,weightSuggest,envKey}`（嵌入文档 §三 四层权重 0.45/0.35/0.15/0.05、舆情建议权重 0.15）、`refresh_opinion_pool` 重写 `backtrace_opinions` 伴随表、`list_opinion_pool` 查询；闭环预警扫描 `scan_alerts` 在 `watchlist=None` 时把舆情池标的作为情绪面筛选叠加（union，与披露叠加正交，引入但不在大涨池的标的好名单登记进 `BacktraceScreenPool` gain_type='舆情催化' 使 agent_dig 可解析），扫描结果新增 `opinionCandidates` 与 per-alert `hasOpinion`；决策权全数学（舆情源只喂情绪催化标的不介入打分）、内核零改动、切换零代码改动仅由环境变量控制、与 #23/#25 正交（行情喂大涨 / 披露喂基本面 / 舆情喂情绪）。
- [新功能] 「公开舆情子系统（#28）」接入 API 与前端：端点 `GET /closed-loop/opinion/source`、`POST /closed-loop/opinion/refresh`、`GET /closed-loop/opinions`；前端 `types/backtrace.ts` 增 `OpinionItem`/`OpinionSourceInfo`（并扩展 `BacktraceSeed.opinions`/`opinionSource`、`ClosedLoopAlert.hasOpinion`、`AlertScanResult.opinionCandidates`）、`api/backtrace.ts` 增 `opinionSource`/`opinionRefresh`/`opinions`、`BacktracePage.tsx` ⑧ Tab 增「头条舆情」badge（实时头条爬虫 / 模拟 + provider 名）+ 头条舆情催化事件池面板（标的/日期/来源/阶段/情绪/热度/谣言降权表，情绪与高热度着色）+ 预警分级看板「舆情催化」列（colSpan 改 12）；权重坚守文档 §六「机构>公告>舆情」，舆情仅作短线情绪因子。
- [测试] 公开舆情子系统（#28）补全端到端验证：`scripts/gen_backtrace_seed.py` 与 `scripts/serve_backtrace.py` 重置表增 `storage.BacktraceOpinion`、刷新/列表/描述接入并断言（沙箱 mode=mock 且 opinionCount>0、refresh count>0、scan 叠加 opinionCandidates≥1、per-alert 含 hasOpinion）；`apps/dsa-web/scripts/verify_backtrace_display.tsx` 增 #28 显示断言（头条舆情 badge / 面板标题 / 说明文案 / 表头 / 谣言降权 / provider 标识 / 预警表舆情催化列）。已验证：后端 `py_compile` + `serve_backtrace.py` 全链路 `ALL_REQUEST_DATA_OK`（opinion source opinionCount=6 rumorCount=1、scan opinionCandidates=6）；前端 esbuild SSR `DISPLAY_OK checks=147`（#28 面板与 badge 全 PASS，138 基线 + 9 项舆情断言）；`docs/CHANGELOG.md` 与 `.workbuddy/memory/2026-08-02.md` 已记录。至此 DSA-PUBLIC-OPINION-V1.0（#28~#30）核心架构集成闭环：可插拔舆情源（mock/Toutiao 优雅回退）+ 伴随表 + 闭环扫描情绪叠加 + API + 前端面板全链路打通，真实环境（DSA_REALTIME_OPINION=1 + 部署 toutiao_spider + FinBERT）可直接接管，沙箱确定性验证零侵入。
<!-- 新条目格式：- [类型] 描述（类型取值：新功能/改进/修复/文档/测试/chore）-->
<!-- 每条独立一行追加到本段末尾，无需分类标题，合并时冲突最小 -->
- [新功能] 反向归因子系统新增「微信私域舆情子系统（DSA-WECHAT-OPINION-V1.0 #31，真实环境适配外延：把与 #28 头条公域舆情平行正交的「私域圈层情绪面催化信号源」从确定性 mock 升级为公众号/视频号爬虫 + FinBERT 可切换源）」：新建 `src/services/wechat_provider.py`，定义统一接口 `BaseWechatProvider`（`get_wechats` 返回标准化 JSON：carrier 载体/ credibility 可信度/ sentiment_score(-1~1)/ heat_score(0~1)/ stage/ has_rumor/ weight_suggest），`MockWechatProvider`（确定性模板，引用大涨池内标的 + 池外 600519 触发 union 登记，覆盖券商公众号/产业垂直号/财经视频号/付费社群线索/其他自媒体载体与高/中/低可信度多样性）与 `WechatSpiderProvider`（真实实现：crawl_service.wechat_spider 公众号 WeChatSpider + 视频号 VideoSpider-WeChat + FinBERT 本地推理 + 可信度分级，缺失/失败抛 `ProviderUnavailable`）；`get_wechat_provider` 工厂按 `DSA_REALTIME_WECHAT=1` 切换、不可用优雅回退 mock；文档 §二/§五 权重**高于头条**（微信短线 0.20 / 长线 0.08 vs 头条 0.15 / 0.05）、§六 权重约束「官方公告>机构研报>微信圈层舆情>头条散户舆情」、合规底线「仅抓公众号公开文章与公开视频号、不碰群聊/朋友圈」均已写入 docstring 与常量；`describe_wechat_source` 返回 `{provider,label,mode,reason,wechatCount,rumorCount,lowCredibilityCount,weightShortSuggest,weightLongSuggest,envKey}`、`refresh_wechat_pool` 重写 `backtrace_wechat_opinions` 伴随表（storage.py 新增 `BacktraceWechatOpinion`，create_all 自动建表）、`list_wechat_pool` 查询；闭环预警扫描 `scan_alerts` 在 `watchlist=None` 时把微信池标的作为私域情绪面筛选叠加（union，与 #25 披露/#28 头条舆情/#23 行情正交，引入但不在大涨池的标的好名单登记进 `BacktraceScreenPool` gain_type='微信舆情' 使 agent_dig 可解析），扫描结果新增 `wechatCandidates` 与 per-alert `hasWechat`。
- [新功能] 「微信私域舆情子系统（#31）」接入 API 与前端：端点 `GET /closed-loop/wechat/source`、`POST /closed-loop/wechat/refresh`、`GET /closed-loop/wechats`；前端 `types/backtrace.ts` 增 `WechatOpinionItem`/`WechatSourceInfo`（并扩展 `BacktraceSeed.wechats`/`wechatSource`、`ClosedLoopAlert.hasWechat`、`AlertScanResult.wechatCandidates`）、`api/backtrace.ts` 增 `wechatSource`/`wechatRefresh`/`wechats`、`BacktracePage.tsx` ⑧ Tab 增「微信舆情」badge（实时公众号/视频号 / 模拟 + provider 名）+ 微信舆情催化事件池面板（标的/日期/载体/可信度/阶段/情绪/热度/谣言降权表，可信度高绿/低红、情绪与高热度着色）+ 预警分级看板「微信催化」列（colSpan 改 13）；权重坚守「微信圈层舆情>头条散户舆情但<官方公告/机构研报」，仅作短线私域情绪因子。
- [测试] 微信私域舆情子系统（#31）补全端到端验证：`scripts/gen_backtrace_seed.py` 与 `scripts/serve_backtrace.py` 重置表增 `storage.BacktraceWechatOpinion`、刷新/列表/描述接入并断言（沙箱 mode=mock 且 wechatCount>0、refresh count>0、scan 叠加 wechatCandidates≥1、per-alert 含 hasWechat；微信权重断言 weightShortSuggest>0.15 且 weightLongSuggest>0.05 以验证高于头条）、`apps/dsa-web/scripts/verify_backtrace_display.tsx` 增 #31 显示断言（微信舆情 badge / 面板标题 / 仅公众号可抓取文案 / 表头载体可信度 / 载体命中 / 高可信 / 谣言降权 / provider 标识 / 预警表微信催化列）。已验证：后端 `py_compile` + `serve_backtrace.py` 全链路 `ALL_REQUEST_DATA_OK`（wechat source wechatCount=6 rumorCount=1 lowCredibilityCount=1、scan wechatCandidates=6）；前端 esbuild SSR `DISPLAY_OK checks=158`（#31 面板与 badge 全 PASS，147 基线 + 11 项微信断言）；`docs/CHANGELOG.md` 与 `.workbuddy/memory/2026-08-02.md` 已记录。至此 DSA-WECHAT-OPINION-V1.0（#31~#33）核心架构集成闭环：可插拔微信舆情源（mock/WeChatSpider 优雅回退）+ 伴随表 + 闭环扫描私域情绪叠加 + API + 前端面板全链路打通，真实环境（DSA_REALTIME_WECHAT=1 + 部署 wechat_spider + FinBERT）可直接接管，沙箱确定性验证零侵入。
- [新功能] 反向归因子系统新增「短线快讯舆情子系统（DSA-FLASH-OPINION-V1.0 #34，真实环境适配外延：把与 #28 头条公域舆情/#31 微信私域舆情平行正交的「短线快讯情绪面催化信号源」从确定性 mock 升级为财联社/华尔街见闻/金十爬虫 + 垂直专业媒体 + FinBERT 可切换源）」：新建 `src/services/flash_provider.py`，定义统一接口 `BaseFlashProvider`（`get_flashes` 返回标准化 JSON：media_type(快讯/深度媒体)/ is_breaking(盘中突发)/ sentiment_score(-1~1)/ heat_score(0~1)/ stage / has_rumor/ weight_suggest），`MockFlashProvider`（确定性模板，引用大涨池内标的 + 池外 600519 触发 union 登记，覆盖财联社/华尔街见闻/金十/券商中国/e公司渠道、快讯 vs 深度媒体、盘中突发、利好/利空/谣言/四阶段多样性）与 `ClsCrawlerProvider`（真实实现：crawl_service.flash_spider 财联社 cls-crawler + 华尔街见闻/金十 Crawl4AI + 垂直媒体 + FinBERT 本地推理 + 谣言降权 80%，缺失/失败抛 `ProviderUnavailable`）；`get_flash_provider` 工厂按 `DSA_REALTIME_FLASH=1` 切换、不可用优雅回退 mock；文档 §一.2/§五.2 权重（财联社短线 0.22 / 长线参考 0.09，短线合并模型维度仅次于圈内前瞻 0.20 与公告落地 0.25）已写入常量与 docstring；`describe_flash_source` 返回 `{provider,label,mode,reason,flashCount,rumorCount,breakingCount,weightShortSuggest,weightLongSuggest,envKey}`、`refresh_flash_pool` 重写 `backtrace_flash_opinions` 伴随表（storage.py 新增 `BacktraceFlashOpinion`，create_all 自动建表）、`list_flash_pool` 查询；闭环预警扫描 `scan_alerts` 在 `watchlist=None` 时把快讯池标的作为短线情绪面筛选叠加（union，与 #25 披露/#28 头条舆情/#31 微信舆情/#23 行情正交，引入但不在大涨池的标的好名单登记进 `BacktraceScreenPool` gain_type='快讯催化' 使 agent_dig 可解析），扫描结果新增 `flashCandidates` 与 per-alert `hasFlash`。
- [新功能] 「短线快讯舆情子系统（#34）」接入 API 与前端：端点 `GET /closed-loop/flash/source`、`POST /closed-loop/flash/refresh`、`GET /closed-loop/flashes`；前端 `types/backtrace.ts` 增 `FlashOpinionItem`/`FlashSourceInfo`（并扩展 `BacktraceSeed.flashes`/`flashSource`、`ClosedLoopAlert.hasFlash`、`AlertScanResult.flashCandidates`）、`api/backtrace.ts` 增 `flashSource`/`flashRefresh`/`flashes`、`BacktracePage.tsx` ⑧ Tab 增「财联社快讯」badge（实时财联社/华尔街见闻/金十 / 模拟 + provider 名）+ 短线快讯催化事件池面板（标的/日期/渠道/类型/突发/阶段/情绪/热度/谣言降权表，深度媒体红/突发热、情绪与高热度着色）+ 预警分级看板「快讯催化」列（colSpan 改 14）；权重坚守「官方公告>机构研报>圈层前瞻>自媒体公域舆情」，财联社作为 A 股短线第一舆情平台短线影响力极强但严禁主导长线基本面。
- [测试] 短线快讯舆情子系统（#34）补全端到端验证：`scripts/gen_backtrace_seed.py` 与 `scripts/serve_backtrace.py` 重置表增 `storage.BacktraceFlashOpinion`、刷新/列表/描述接入并断言（沙箱 mode=mock 且 flashCount>0、refresh count>0、scan 叠加 flashCandidates≥1、per-alert 含 hasFlash；快讯权重断言 weightShortSuggest==0.22 且 weightLongSuggest==0.09 以对齐文档 §一.2/§五.2）、`apps/dsa-web/scripts/verify_backtrace_display.tsx` 增 #34 显示断言（财联社快讯 badge / 面板标题 / 说明文案 / 表头类型突发 / 深度媒体 / 盘中突发 / 谣言降权 / provider 标识 / 预警表快讯催化列）。已验证：后端 `py_compile` + `serve_backtrace.py` 全链路 `ALL_REQUEST_DATA_OK`（flash source flashCount=6 rumorCount=1 breakingCount=3、scan flashCandidates=6）；前端 esbuild SSR `DISPLAY_OK checks=169`（#34 面板与 badge 全 PASS，158 基线 + 11 项快讯断言）；`docs/CHANGELOG.md`、`docs/backtrace-opinion-architecture.md` 与 `.workbuddy/memory/2026-08-02.md` 已记录。至此 DSA 反向归因系统已建成「行情源(#23)/披露源(#25)/头条舆情(#28)/微信舆情(#31)/短线快讯(#34)」五路正交可插拔信号源架构；剩余蓝图待建：社区源(雪球+股吧+淘股吧)、海外源(彭博/路透/Seeking Alpha)、六层信息圈层模型与多源交叉验证(P1)、舆情回测与拐点预警(P2)。
- [新功能] 反向归因子系统新增「K 线技术面算力底座（DSA-KRONOS-V1.0 #35，真实环境适配外延：把清华大学开源 AAAI2026 金融 K 线基础大模型 Kronos/shiyu-coder/Kronos + NeoQuasar 权重 作为 DSA 的【技术面统一算力底座】从确定性 mock 升级为真实模型推理）」：与 #25/#28/#31/#34「事件/情绪催化源（union 叠加候选池）」本质不同——Kronos 是**逐只股票的技术面算力底座**，不对扫描候选池做 union 扩张，而是对每只已扫描 alert **富化技术面信号**，并独立输出三类选股池；新建 `src/services/kronos_service.py`，定义 `BaseKronosProvider`（`analyze` 返回标准化 JSON：trend 多头/空头/震荡、momentum 趋势强度、inflectionPoint 拐点、riseProb/sidewayProb/downProb 三态概率、volatility、volumeScore、persistence、factorScores 候选 Alpha 因子）、`MockKronosProvider`（按股票代码 hash 确定性分桶，覆盖强势多头/高位顶部风险/底部反转三类场景）、`KronosModelProvider`（真实实现：NeoQuasar 权重 + BSQ 球面量化 Tokenizer + 分层因果 Transformer 自回归推理，缺失 torch/transformers/GPU 抛 `ProviderUnavailable`）；`get_kronos_provider` 工厂按 `DSA_REALTIME_KRONOS=1` 切换、不可用优雅回退 mock；`describe_kronos_source` 返回 `{provider,label,mode,reason,modelFamily,modelSpec,contextWindow,weightShortCap,weightLongCap,analyzedCount,envKey}`、`refresh_kronos`（重写 `backtrace_kronos_signals` 伴随表，storage.py 新增 `BacktraceKronosSignal`，create_all 自动建表）、`list_kronos`、`kronos_pools` 输出三类选股池（短线强势池 riseProb≥0.7 多头 / 趋势反转池 底部拐点 / 风险预警池 顶部拐点或 downProb≥0.5）；`scan_alerts` 对每只 alert 富化 `kronosInfo`（缓存优先、未命中惰性 analyze_stock 实时分析）并新增结果 `kronosAnalyzed`（**不做** union 候选池叠加，决策权坚守）。
- [新功能] 「K 线技术面算力底座（#35）」接入 API 与前端：端点 `GET /closed-loop/kronos/source`、`POST /closed-loop/kronos/refresh`、`GET /closed-loop/kronos/signals`、`GET /closed-loop/kronos/pools`；前端 `types/backtrace.ts` 增 `KronosSignal`/`KronosSourceInfo`/`KronosPools`（并扩展 `BacktraceSeed.kronosSource`/`kronosPools`、`ClosedLoopAlert.kronosInfo`、`AlertScanResult.kronosAnalyzed`）、`api/backtrace.ts` 增 `kronosSource`/`kronosRefresh`/`kronosSignals`/`kronosPools`、`BacktracePage.tsx` ⑧ Tab 增「Kronos 技术面」badge（实时 NeoQuasar small / 模拟 + provider 名）+ 算力底座面板（模型族/上下文窗口/已分析数/K线权重上限 短线0.35·长线0.15 + 三段风控约束声明 + 三类选股池：短线强势池/趋势反转池/风险预警池）+ 预警分级看板「Kronos趋势/拐点/Kronos涨概」三列（colSpan 改 17）；权重硬上限坚守蓝图 §七（K线信号短线最高0.35、长线最高0.15，基本面与公告优先级恒最高，Kronos 仅输出技术参考、最终涨跌量化与中长期结论由 DSA 数学模型决定）。
- [测试] K 线技术面算力底座（#35）补全端到端验证：`scripts/gen_backtrace_seed.py` 与 `scripts/serve_backtrace.py` 重置表增 `storage.BacktraceKronosSignal`、刷新/描述/选股池接入并断言（沙箱 mode=mock 且 analyzed=14、refresh analyzed≥1、pools 三类非空；权重硬上限断言 weightShortCap==0.35 且 weightLongCap==0.15 以对齐蓝图 §七；scan 逐 alert 富化 kronosInfo 且 kronosAnalyzed==14）、`apps/dsa-web/scripts/verify_backtrace_display.tsx` 增 #35 显示断言（Kronos 技术面 badge / 算力底座面板标题 / 风控约束声明 / 三类选股池标题 / 预警表 Kronos趋势·拐点·涨概三列）；已验证：后端 `py_compile` + `serve_backtrace.py` 全链路 `ALL_REQUEST_DATA_OK`（kronosAnalyzed=14、pools 短线强势1/反转5/风险9）；前端 esbuild SSR `DISPLAY_OK checks=178`（#35 面板与 badge 全 PASS，169 基线 + 9 项 Kronos 断言）；`docs/CHANGELOG.md`、`docs/backtrace-kronos-architecture.md` 与 `.workbuddy/memory/2026-08-02.md` 已记录。至此 DSA 反向归因系统已建成「行情源(#23)/披露源(#25)/头条舆情(#28)/微信舆情(#31)/短线快讯(#34)/K线技术面(#35)」六路正交可插拔信号源架构；Kronos 与事件/情绪/披露源正交互补——前者提供逐 alert 技术面算力（趋势/拐点/三态概率/波动率/量能/Alpha 因子）且不扩张候选池，后者负责事件/情绪催化 union 叠加候选标的。
- [新功能] 反向归因子系统新增「深度社区舆情子系统（DSA-COMMUNITY-OPINION-V1.0 #36，真实环境适配外延：把与 #28 头条公域舆情/#31 微信私域舆情/#34 短线快讯平行正交的「深度社区情绪面催化信号源」从确定性 mock 升级为雪球/东财股吧/淘股吧爬虫 + 质量分层 + FinBERT 可切换源）」：新建 `src/services/community_provider.py`，定义统一接口 `BaseCommunityProvider`（`get_community_posts` 返回标准化 JSON：platform(雪球/东财股吧/淘股吧)/ quality(高质量/普通/噪音)/ is_hot(登社区热榜)/ post_count/ discussion_heat(0~1)/ sentiment_score(-1~1)/ sentiment(看多/中性/看空)/ has_rumor/ weight_suggest），`MockCommunityProvider`（确定性模板，引用大涨池内标的 + 池外 600519 触发 union 登记，覆盖雪球高质量长文/股吧散户情绪放大器/淘股吧游资打板三类平台与高质量 vs 噪音质量分层、登热榜、看多/看空/谣言多样性）与 `CommunityCrawlerProvider`（真实实现：crawl_service.community_spider 雪球/东财股吧/淘股吧异步爬虫 + FinBERT 本地推理 + 质量分层（雪球=高质量、股吧·淘股吧=噪音）+ 谣言降权 80%，缺失/失败抛 `ProviderUnavailable`）；`get_community_provider` 工厂按 `DSA_REALTIME_COMMUNITY=1` 切换、不可用优雅回退 mock；文档 §一.2/§五 权重（社区讨论短线 0.13 / 长线参考 0.05，社区对中长线影响弱；§五.2 短线合并模型未纳入社区维度，按 §一.2 参考值展示）已写入常量与 docstring；`describe_community_source` 返回 `{provider,label,mode,reason,communityCount,rumorCount,hotCount,weightShortSuggest,weightLongSuggest,envKey}`、`refresh_community_pool` 重写 `backtrace_community_opinions` 伴随表（storage.py 新增 `BacktraceCommunityOpinion`，create_all 自动建表）、`list_community_pool` 查询；闭环预警扫描 `scan_alerts` 在 `watchlist=None` 时把社区池标的作为社区情绪面筛选叠加（union，与 #25 披露/#28 头条舆情/#31 微信舆情/#34 短线快讯/#23 行情正交，引入但不在大涨池的标的好名字登记进 `BacktraceScreenPool` gain_type='社区热议' 使 agent_dig 可解析），扫描结果新增 `communityCandidates` 与 per-alert `hasCommunity`。
- [新功能] 「深度社区舆情子系统（#36）」接入 API 与前端：端点 `GET /closed-loop/community/source`、`POST /closed-loop/community/refresh`、`GET /closed-loop/communities`；前端 `types/backtrace.ts` 增 `CommunityOpinionItem`/`CommunitySourceInfo`（并扩展 `BacktraceSeed.communities`/`communitySource`、`ClosedLoopAlert.hasCommunity`、`AlertScanResult.communityCandidates`）、`api/backtrace.ts` 增 `communitySource`/`communityRefresh`/`communities`、`BacktracePage.tsx` ⑧ Tab 增「深度社区舆情」badge（实时雪球/股吧/淘股吧 / 模拟 + provider 名）+ 深度社区舆情催化事件池面板（标的/日期/平台/质量/热榜/情绪/热度/谣言降权表，雪球高质量绿/股吧淘股吧噪音红、登热榜黄、情绪与高热度着色）+ 预警分级看板「社区热议」列（colSpan 改 18）；权重坚守「官方公告>机构研报>圈层前瞻>社区情绪>自媒体公域舆情」，社区平台对散户情绪/短线题材/追涨杀跌/谣言发酵影响力极强但严禁主导长线基本面。
- [测试] 深度社区舆情子系统（#36）补全端到端验证：`scripts/gen_backtrace_seed.py` 与 `scripts/serve_backtrace.py` 重置表增 `storage.BacktraceCommunityOpinion`、刷新/列表/描述接入并断言（沙箱 mode=mock 且 communityCount>0、refresh count>0、scan 叠加 communityCandidates≥1、per-alert 含 hasCommunity；社区权重断言 weightShortSuggest==0.13 且 weightLongSuggest==0.05 以对齐文档 §一.2）、`apps/dsa-web/scripts/verify_backtrace_display.tsx` 增 #36 显示断言（深度社区舆情 badge / 面板标题 / 说明文案 / 表头平台质量 / 雪球高质量 / 登热榜 / 谣言降权 / provider 标识 / 预警表社区热议列）。已验证：后端 `py_compile` + `serve_backtrace.py` 全链路 `ALL_REQUEST_DATA_OK`（community source communityCount=6 rumorCount=1 hotCount=3、scan communityCandidates=6）；前端 esbuild SSR `DISPLAY_OK checks=189`（#36 面板与 badge 全 PASS，178 基线 + 11 项社区断言）；`docs/CHANGELOG.md` 与 `.workbuddy/memory/2026-08-02.md` 已记录。至此 DSA 反向归因系统已建成「行情源(#23)/披露源(#25)/头条舆情(#28)/微信舆情(#31)/短线快讯(#34)/深度社区(#36)/K线技术面(#35)」七路正交可插拔信号源架构。
- [新功能] 反向归因子系统新增「海外权威舆情子系统（DSA-OVERSEAS-OPINION-V1.0 #37，真实环境适配外延：把与 #25/#28/#31/#34/#36 平行正交的「海外权威情绪面催化信号源」从确定性 mock 升级为彭博/路透/WSJ/Seeking Alpha 抓取 + 机构评级 + 外资流向可切换源）」：新建 `src/services/overseas_provider.py`，定义统一接口 `BaseOverseasProvider`（`get_overseas_news` 返回标准化 JSON：platform(彭博/路透/WSJ/Seeking Alpha)/ region(海外)/ is_institution(机构评级·研报)/ rating(增持/中性/减持/无)/ sentiment_score(-1~1)/ sentiment(看多/中性/看空)/ impact_type(外资流向/评级调整/基本面/宏观)/ weight_suggest），`MockOverseasProvider`（确定性模板，引用大涨池内标的 + 池外 600519 触发 union 登记，覆盖彭博机构评级上调/路透外资流入/WSJ 基本面/Seeking Alpha 多空论点四类平台及机构评级、看多/看空、催化类型多样性）与 `OverseasCrawlerProvider`（真实实现：crawl_service.overseas_spider Bloomberg/Reuters/WSJ/SeekingAlpha 抓取 + 机构评级/外资流向解析，缺失/失败抛 `ProviderUnavailable`）；`get_overseas_provider` 工厂按 `DSA_REALTIME_OVERSEAS=1` 切换、不可用优雅回退 mock；文档 §一.6/§五.2 权重（海外短线 0.14 / 长线外资维度 0.18，§五.2 长线合并模型保留彭博/路透系，华尔街见闻/金十长线 0.11 未进入长线合并模型）已写入常量与 docstring；`describe_overseas_source` 返回 `{provider,label,mode,reason,overseasCount,institutionCount,ratingUpCount,weightShortSuggest,weightLongSuggest,envKey}`、`refresh_overseas_pool` 重写 `backtrace_overseas_opinions` 伴随表（storage.py 新增 `BacktraceOverseasOpinion`，create_all 自动建表）、`list_overseas_pool` 查询；闭环预警扫描 `scan_alerts` 在 `watchlist=None` 时把海外池标的作为海外权威情绪面筛选叠加（union，与 #25/#28/#31/#34/#36/#23 正交，引入但不在大涨池的标的登记进 `BacktraceScreenPool` gain_type='海外权威'），扫描结果新增 `overseasCandidates` 与 per-alert `hasOverseas`。
- [新功能] 「海外权威舆情子系统（#37）」接入 API 与前端：端点 `GET /closed-loop/overseas/source`、`POST /closed-loop/overseas/refresh`、`GET /closed-loop/overseas`；前端 `types/backtrace.ts` 增 `OverseasNewsItem`/`OverseasSourceInfo`（并扩展 `BacktraceSeed.overseas`/`overseasSource`、`ClosedLoopAlert.hasOverseas`、`AlertScanResult.overseasCandidates`）、`api/backtrace.ts` 增 `overseasSource`/`overseasRefresh`/`overseas`、`BacktracePage.tsx` ⑧ Tab 增「海外权威」badge（实时彭博/路透/WSJ/Seeking Alpha / 模拟 + provider 名）+ 海外权威资讯催化事件池面板（标的/日期/平台/机构/评级/情绪/催化类型表，机构绿、看多绿/看空红、催化类型着色）+ 预警分级看板「海外权威」列（colSpan 改 19）；权重坚守「官方公告>机构研报>圈层前瞻>自媒体公域>社区情绪」，海外权威源主要作用于长线外资维度（权重 0.18，外资定价权与北向资金风向标），严禁主导短线题材。
- [测试] 海外权威舆情子系统（#37）补全端到端验证：`scripts/gen_backtrace_seed.py` 与 `scripts/serve_backtrace.py` 重置表增 `storage.BacktraceOverseasOpinion`、刷新/列表/描述接入并断言（沙箱 mode=mock 且 overseasCount>0、refresh count>0、scan 叠加 overseasCandidates≥1、per-alert 含 hasOverseas；海外权重断言 weightShortSuggest==0.14 且 weightLongSuggest==0.18 以对齐文档 §一.6/§五.2）、`apps/dsa-web/scripts/verify_backtrace_display.tsx` 增 #37 显示断言（海外权威 badge / 面板标题 / 说明文案 / 表头平台机构评级 / 彭博平台 / provider 标识 / 预警表海外权威列）。已验证：后端 `py_compile` + `serve_backtrace.py` 全链路 `ALL_REQUEST_DATA_OK`（overseas source overseasCount=6 institutionCount=4 ratingUpCount=3、scan overseasCandidates=6）；前端 esbuild SSR `DISPLAY_OK checks=198`（#37 面板与 badge 全 PASS，189 基线 + 9 项海外断言）；`docs/CHANGELOG.md` 与 `.workbuddy/memory/2026-08-02.md` 已记录。至此 DSA 反向归因系统已建成「行情源(#23)/披露源(#25)/头条舆情(#28)/微信舆情(#31)/短线快讯(#34)/深度社区(#36)/海外权威(#37)/K线技术面(#35)」八路正交可插拔信号源架构。
- [新功能] 反向归因子系统新增「六层信息圈层 + 多源交叉验证（DSA-OPINION-CROSS-V1.0 #38，元分析层）」：区别于 #34~#37 的「信号源」实现，本项把已落地的七路源归入蓝图 §四 的 L0~L5 可信圈层（L0 披露/法定权威、L1 海外/机构、L2 快讯+雪球高质量、L3 微信私域+股吧淘股吧、L4 头条公域散户），并实现 §五.1 多源交叉验证规则。新建 `src/services/opinion_info_layers.py`（INFO_LAYERS(L0~L5) + AUTHORITATIVE/RETAIL/PROFESSIONAL 分组 + SOURCE_LAYER_MAP(七路源→圈层) + §4 可信度阈值 singleRetailCap=0.30/singleAuth=0.50/multiAuthFloor=0.70/multiAuthCeil=0.90 + TIER_DIRECTION_WEIGHT + describe_info_layers()）；`src/services/opinion_cross_validation.py`（build_source_index 一次性读七路表 per-stock 情感/可信度/谣言建内存索引；cross_validate_alert 计算 圈层命中/独立权威源数(披露1+海外去重平台)/共识等级/可信度/加权净方向/权威×散户冲突/谣言标记；aggregate_summary/cross_validate_scan/summarize_over_pools）。`closed_loop_scan_service.scan_alerts` 在 alert 构建后附加 per-alert `crossValidation` 并在结果 dict 增 `crossValidationSummary`（不改 union 逻辑、不改内核决策权）；`backtrace.py` 增端点 `GET /closed-loop/info-layers`、`GET /closed-loop/cross-validation`，更新 docstring 端点列表。
- [新功能] 「六层信息圈层 + 多源交叉验证（#38）」接入前端：`types/backtrace.ts` 增 `CrossValidation`/`CrossValidationSummary`/`InfoLayers`（并扩展 `BacktraceSeed.crossValidationSummary`/`infoLayers`、`ClosedLoopAlert.crossValidation`、`AlertScanResult.crossValidationSummary`）、`api/backtrace.ts` 增 `infoLayers`/`crossValidation`、`BacktracePage.tsx` 增「六层信息圈层 + 多源交叉验证」面板（L0~L5 圈层命中矩阵 + 共识分布 strong/moderate/weak/none + 多源确认/权威×散户冲突/谣言待甄别/#35 技术面多头确认指标 + §4 可信度阈值说明）+ 预警分级看板「共识/可信度」列（colSpan 改 20，显示共识 badge + 可信度分数 + 冲突/谣言标记）。
- [测试] 六层信息圈层 + 多源交叉验证（#38）补全端到端验证：`scripts/gen_backtrace_seed.py` 接入 `describe_info_layers`（种子 `infoLayers`）+ 扫描结果 `crossValidationSummary`（种子 `crossValidationSummary`）；`scripts/serve_backtrace.py` 断言扫描结果含 `crossValidationSummary`（layerDistribution 六键齐全、consensusDistribution 四键齐全、multiSourceConfirmed≥1、per-alert 含 crossValidation 且 credibilityScore∈[0,1]）并新增端点断言（/info-layers 返回六层 + singleRetailCap==0.3、/cross-validation totalAlerts≥1）；`apps/dsa-web/scripts/verify_backtrace_display.tsx` 增 #38 显示断言（面板标题 / 圈层矩阵六层标签 / 强共识标签 / 权威×散户冲突 / 谣言待甄别 / §4 可信度阈值说明 / 预警表共识列 / 共识 badge）。已验证：后端 `py_compile` + `serve_backtrace.py` 全链路 `ALL_REQUEST_DATA_OK`（multiSourceConfirmed=6、consensus strong:6/none:8、conflict:0、rumor:1）；前端 esbuild SSR `DISPLAY_OK checks=206`（#38 面板与列全 PASS，198 基线 + 8 项交叉验证断言）；`docs/CHANGELOG.md` 与 `docs/backtrace-opinion-architecture.md` 与 `.workbuddy/memory/2026-08-02.md` 已记录。至此 DSA 反向归因系统在八路正交信号源之上叠加了「六层信息圈层 + 多源交叉验证」元分析层（蓝图 §四/§五.1 闭环）。
- [新功能] 反向归因子系统新增「舆情回测 + 拐点预警（DSA-OPINION-BACKTEST-V1.0 #39，元分析层，P2）」：与 #38 同构（非信号源、不改 union、不改内核决策权），把蓝图 P2 落地。新建 `src/services/opinion_backtest.py`：（1）舆情回测——对六路可插拔源(#25 披露/#28 头条/#31 微信/#34 快讯/#36 社区/#37 海外)构造确定性历史情绪序列 + 共享隐藏「真实涨跌因子」f，真实计算方向胜率/多头胜率/空头胜率/IC(秩相关)/覆盖率/可靠性分级（权威源耦合强→IC 高、散户噪声大→弱/反向，结论由模拟序列真实算出而非硬编码）；（2）拐点预警——`detect_inflection_for_alert` 消费 #38 交叉验证 + 回测可靠性 + #35 Kronos 技术面，判定见顶拐点(散户集中看多无权威印证)/启动拐点(权威提前看多散户未跟进)/情绪反转(主导散户源 IC 偏弱当前一致看多)/技术·情绪背离(技术面与舆情背离)四类，输出 level/types/reasons/confidence/suggestedAction；`summarize_inflection` 聚合扫描级 levelDistribution/typeDistribution/highInflectionAlerts。`closed_loop_scan_service.scan_alerts` 在 #38 块后计算 `run_sentiment_backtest(universe)`、逐 alert 附加 `inflectionWarning`、结果 dict 增 `inflectionSummary`；`backtrace.py` 增端点 `GET /closed-loop/sentiment-backtest`、`GET /closed-loop/inflection-warnings`，更新 docstring。
- [新功能] 「舆情回测 + 拐点预警（#39）」接入前端：`types/backtrace.ts` 增 `SentimentBacktestItem`/`SentimentBacktestSummary`/`SentimentBacktestReport`/`InflectionWarning`/`InflectionSummary`（并扩展 `BacktraceSeed.sentimentBacktest`/`inflectionSummary`、`ClosedLoopAlert.inflectionWarning`、`AlertScanResult.inflectionSummary`）、`api/backtrace.ts` 增 `sentimentBacktest`/`inflectionWarnings`、`BacktracePage.tsx` 增「舆情回测 + 拐点预警」面板（各平台情绪因子历史胜率表：平台/层级/样本/方向胜率/多头胜率/空头胜率/IC/信号方向/可靠性 + 最强/最弱源摘要 + 拐点预警摘要：高危/中/低/无分级 + 命中类型分布 + 高危标的表）+ 预警分级看板「拐点预警」列（colSpan 改 21，显示拐点 badge + 类型）。
- [测试] 舆情回测 + 拐点预警（#39）补全端到端验证：`scripts/gen_backtrace_seed.py` 接入 `sentiment_backtest_over_pools`（种子 `sentimentBacktest`）+ 扫描结果 `inflectionSummary`（种子 `inflectionSummary`）；`scripts/serve_backtrace.py` 断言扫描结果含 `inflectionSummary`（levelDistribution 四键齐全、per-alert 含 inflectionWarning 且 confidence∈[0,1]）并新增端点断言（/sentiment-backtest 六源齐全、方向胜率∈[0,1]、IC∈[-1,1]、samples>0、reliability∈{高,中,低}、bestIc≥worstIc；/inflection-warnings levelDistribution 四键齐全、totalAlerts≥1）；`apps/dsa-web/scripts/verify_backtrace_display.tsx` 增 #39 显示断言（面板标题 / 方向胜率列 / IC 列 / 可靠性列 / 最强预测源摘要 / 拐点预警摘要 / 技术·情绪背离类型 / 预警表拐点预警列）。已验证：后端 `py_compile` + `serve_backtrace.py` 全链路 `ALL_REQUEST_DATA_OK`（sentiment-backtest best=disclosure IC=0.53、worst=opinion IC=0.04、authoritativeAvgIc=0.50>retailAvgIc=0.17；inflection scan 命中 2 例「技术·情绪背离」medium）；前端 esbuild SSR `DISPLAY_OK checks=214`（#39 面板与列全 PASS，206 基线 + 8 项回测/拐点断言）；`docs/CHANGELOG.md` 与 `docs/backtrace-opinion-architecture.md` 与 `.workbuddy/memory/` 已记录。至此 DSA 反向归因系统在八路正交信号源 + 六层圈层交叉验证之上，进一步叠加「舆情回测(各平台情绪因子历史胜率) + 拐点预警」元分析层（蓝图 P2 闭环）。
...
- [文档] 新增长文本深度解析模块整合落地方案 `docs/llm-deep-parse-integration.md`（DSA-OPT-LLM-001 V1.0）：经 GitHub 真实核查修订 vibe-research/FinGPT/LangChain 复用规划，修正原草稿中不存在的 PDF-Query-Fin 并替换为 CiteOrDie/PactGuard-ERNIE-PP/contractex，增补 FinSight(RUC) 为首选参考；明确四层架构、标准化 JSON 对接层、5 项能力排期、与 `llm/gateway.py`/`runtime_scheduler.py`/`industry_chain_propagation.py` 的接入点及「请求→数据→显示」验证范式
- [新功能] 长文本解析弹窗升级为四模式 Tab（DSA-OPT-LLM-001 Phase 2 收尾）：`components/llm-parse/LlmParseModal.tsx` 支持「分层拆解 / 多文档对比 / 隐藏约束 / 长期规划」四模式，`ParseResultView` 新增隐藏约束与长期规划两种渲染分支；`types/llmParse.ts` 增补 `ConstraintData`/`LongTermData`；`scripts/verify_llm_parse_display.tsx` 全链路显示验证（checks=10）。
- [新功能] 产业链维护收尾（设计 §5.3 / 页面4）：`api/v1/endpoints/industry_chain.py` 新增 `PUT /{id}/edge-override`（自定义传导系数，clamp 0~1）、`GET /{id}/edge-overrides`、`POST /{id}/risk-flag`、`GET /{id}/risk-flags`、`GET /{id}/export-template`（画布模板导出），落库 `chain_edge_override` / `chain_risk_flag` 表；前端新增 `types/industryChain.ts`、`api/industryChain.ts`、`pages/IndustryMaintenancePage.tsx` 并注册 `/industry-maintenance` 路由与侧边导航（含 zh/en i18n）；`scripts/serve_maintenance.py` + `scripts/verify_maintenance_display.tsx` 验证「请求→数据→显示」全链路。
- [新功能] 公司维护收尾（设计 §5.3 / 页面5）：`api/v1/endpoints/company.py` 新增 `GET/POST /companies/{code}/risk-tags`（启发式利好/利空关键词识别并写库 `company_risk_tags` 表），`GET /companies/{code}` 合并返回 `riskTags`；前端新增 `types/company.ts`、`api/company.ts`、`pages/CompanyMaintenancePage.tsx`（基础/财务/关联产业链/风险 四 Tab + 风险高亮 + 自动识别写库）并注册 `/company-maintenance` 路由与侧边导航（含 zh/en i18n）；同 `scripts/verify_maintenance_display.tsx` 验证全链路。
- [新功能] DSA 全局模型参数管控（设计 §5.3）：新增 `api/v1/endpoints/dsa_params.py`（`GET /`、`POST /seed`、`PUT /{key}`，操作 `dsa_global_params` 表：递归深度/系数阈值/风险衰减等），注册 `/api/v1/dsa-params`；前端新增 `types/dsaParams.ts`、`api/dsaParams.ts`、`pages/DsaParamsPage.tsx` 并注册 `/dsa-params` 路由与侧边导航（含 zh/en i18n）；同 `scripts/verify_maintenance_display.tsx` 验证全链路。
- [新功能] 前瞻预测中心（设计 §3.2 / §5.3 表1）：新增 `api/v1/endpoints/forecast_snapshot.py`（`GET /` 按 scope_type/cycle 过滤 + 四周期聚合概览、`POST /seed` 写入确定性占位四周期快照，落库 `forecast_batch_snapshot`），注册 `/api/v1/forecast-snapshots`；前端新增 `types/forecastSnapshot.ts`、`api/forecastSnapshot.ts`、`pages/ForecastCenterPage.tsx`（四周期×多 scope 卡片 + 范围筛选 + 演示快照生成）并注册 `/forecast-center` 路由与侧边导航（TrendingUp 图标，含 zh/en i18n）；`scripts/serve_forecast_scheduler.py` + `scripts/verify_forecast_scheduler_display.tsx` 验证全链路（请求→数据→显示，checks=12）。
- [新功能] 调度运行日志（设计 §5.3 表4）：`api/v1/endpoints/scheduler.py` 扩展 `POST /jobs/{id}/record`（记录一次运行到 `scheduler_job_run` 表）、`GET /runs`（最近运行日志）；`SchedulerAdminPage` 联动加载并在 `JobBoard` 新增「最近运行日志」分区（成功/失败/运行中状态色标）；前端 `types/scheduler.ts`、`api/scheduler.ts` 增补 `SchedulerJobRun` 与 `getRuns`/`recordRun`；同 `scripts/serve_forecast_scheduler.py` + `scripts/verify_forecast_scheduler_display.tsx` 验证全链路。至此 §5.3「四张新表」全部闭环（chain_edge_override / dsa_global_params / forecast_batch_snapshot / scheduler_job_run）。
- [新功能] 情报结构化 5 字段 + AI 分级（设计 §2.2 / §5.2）：新增 `api/v1/endpoints/intelligence_impact.py`（`POST /grade` 接受情报条目并启发式计算 impact_level/impact_cycle/impact_industry/impact_direction/transmit_weight 后 upsert、`GET /impacts` 按方向/等级过滤，落库外挂伴随表 `intelligence_item_impact`，不改既有 intelligence_items 宽表），注册 `/api/v1/intelligence-impact`；前端新增 `types/intelligenceImpact.ts`、`api/intelligenceImpact.ts`、`components/IntelligenceImpactPanel.tsx`（5 字段展示，方向色标 + 传导权重）并接入 `DynamicsCenterPage` 右栏；`scripts/serve_intelligence_impact.py` + `scripts/verify_intelligence_impact_display.tsx` 验证全链路（请求→数据→显示，checks=8）。设计文档 §8 第一阶段全部缺口闭合。
- [新功能] 自动爬虫 + 长文本解析流水线 P0（DSA-CRAWL-LLM-MERGE-V1.0）：新增 `src/services/crawl_service.py`（抓取源注册表 + 确定性 mock 供料 → 复用 `core/llm_parse_service` 解析 → 落库 `crawled_documents` 伴随表，真实适配器按需接入 cninfo/astock/Crawl4AI）、`api/v1/endpoints/crawl.py`（`GET /sources`、`POST /run`、`GET /documents`，注册 `/api/v1/crawl`），以及 `normalize_parsed` 将解析层嵌套 snake_case 输出扁平化为前端 camelCase 契约；前端新增 `types/crawl.ts`、`api/crawl.ts`、`pages/CrawlCenterPage.tsx`（抓取源卡片 + 一键抓取解析 + 结构化结果列表）并注册 `/crawl-center` 路由与侧边导航（Download 图标，含 zh/en i18n）；`scripts/serve_crawl.py` + `scripts/verify_crawl_display.tsx` 验证全链路（请求→数据→显示，checks=10）。沙箱无外网，走 mock 供料，真实适配器按同契约替换。
