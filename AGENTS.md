...
用户要求：**每次项目更改提交到本地 git；每次需求更改持久化到本地需求知识库 RAG。**

- **代码改动**：在完成任务、验证通过后，将本次相关的源码改动提交到本地 git（`main` 分支，仅本地、不推送）。commit message 用英文、不添加 `Co-Authored-By`。提交范围须最小、可归因，严格排除 `config.yaml`、`.env`（`.env.example` 例外）、`*.db*`、`litellm_config.yaml` 等敏感/大文件及与本次需求无关的历史改动；如工作树存在大范围无关脏改动，先只提交本次相关文件并向用户说明其余待提交项。
- **需求改动**：每次需求变更，在 `knowledge_base/requirements/` 下新增/更新一份 Markdown 需求文档（约定 front-matter：`**日期**`/`**需求**`/`**状态**`），随后运行 `python -m src.rag.requirements_kb rebuild`（或 ingest）重建本地 BM25+SQLite 索引（`knowledge_base/requirements_kb.db`，gitignore、可重建）。检索 API：`src/rag/requirements_kb.py` 的 `RequirementsKB.query(text, top_k)`。
- 需求文档随 git 版本管理（即需求既进 RAG 也进版本库）；RAG 索引为构建产物不入库。
