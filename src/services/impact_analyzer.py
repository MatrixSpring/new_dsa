...
from src.data.industry_chain_fusion import match_shenwan_by_text
...
def analyze_article_impact(article: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    # 寻找申万产业链中的关联（中文感知匹配，修复原空白断链）
    shenwan_chains = match_shenwan_by_text(f"{title} {content}", top_n=5)
