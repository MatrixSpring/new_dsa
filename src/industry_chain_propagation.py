...
def propagate_shock(graph: Dict[str, Any], shock: Dict[str, Any]) -> Dict[str, Any]:
    """沿图谱传播冲击，返回各环节/各公司影响与汇总。

    Args:
        graph: 产业链图谱(dict)，含 nodes/edges/companies（同 /industry-chains/{id} 返回）
        shock: {node(环节 id 或名称), magnitude(相对冲击, 如 -0.2=跌20%/成本+20%),
                kind(cost/demand/supply/substitute，仅用于说明)}
    Returns:
        {shock_node, shock_label, magnitude, kind,
    ...
    while q:
        cur = q.popleft()
        for nb, e in adj[cur]:
            coeff = float(e.get("coeff", 0.6) or 0.6)
            lag = float(e.get("lag", 0) or 0)
            decay = 1.0 / (1.0 + lag / 30.0)          # 滞后越长，即时冲击越弱
            child = impact[cur] * coeff * decay
            if nb not in impact:
                impact[nb] = child
                if abs(child) > 1e-4:
                    q.append(nb)
