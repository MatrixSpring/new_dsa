...
class BaseFetcher(ABC):
    ...
    # --- 共享 HTTP 客户端（子类复用，避免频繁创建连接） ---
    _http_client: Optional[Any] = None
    _http_lock: Optional[Any] = None

    @property
    def source_name(self) -> str:
        """数据源短标识名（用于路由和日志，默认取类名）"""
        return self.name

    def _ensure_http_lock(self):
        if self._http_lock is None:
            import threading
            self._http_lock = threading.Lock()

    def _get_or_create_http_client(self, timeout: float = 15.0):
        """获取复用的同步 http 客户端（requests.Session 线程安全）"""
        self._ensure_http_lock()
    ...
    # ============================================================
    # Phase 1-3 新增抽象采集契约（舆情 / 基本面 / 新闻 / 筹码）
    # ============================================================
