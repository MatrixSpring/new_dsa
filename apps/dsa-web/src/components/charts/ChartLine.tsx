...
function renderSvgChart(container: HTMLDivElement, data: Record<string, any>[], series: SeriesConfig[]) {
  ...
  series.forEach((s, i) => {
    ...
    const safeColor = escapeHtml(String(s.color));
    legend += `<rect x="${lx}" y="${h - 18}" width="12" height="12" rx="2" fill="${safeColor}" />`;
    legend += `<text x="${lx + 16}" y="${h - 7}" fill="#94A3B8" font-size="11">${escapeHtml(String(s.name))}</text>`;
  });
  ...
  container.innerHTML = `
    <svg width="${w}" height="${h}" xmlns="http://www.w3.org/2000/svg">
      ...
      ${legend}
      ${series.map((s, i) =>
        `<path d="${paths[i]}" fill="none" stroke="${escapeHtml(String(s.color))}" stroke-width="2" stroke-linejoin="round" />`
      ).join('')}
      ${data.map((_, i) =>
        `<circle cx="${sx(i).toFixed(1)}" cy="${sy(Number(data[i][series[0].dataKey]) || 0).toFixed(1)}" r="3" fill="${escapeHtml(String(series[0].color))}" opacity="0.6" />`
      ).join('')}
    </svg>
  `;
}
