...
def _run_single_model(model_name: str, _inputs: dict) -> dict:
    ...
    # 使用确定性哈希生成稳定占位值，避免多 worker 下结果不一致
    import hashlib
    h = int(hashlib.md5(model_name.encode()).hexdigest(), 16)
    score = round(0.35 + (h % 3000) / 10000, 4)       # 0.35 ~ 0.65
    confidence = round(0.55 + (h % 3500) / 10000, 4)    # 0.55 ~ 0.90
